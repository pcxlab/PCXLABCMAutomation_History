function Add-PCXCMCollectionExclusion {
    ...
    process {
        try {
            $TargetCollection = Get-CMDeviceCollection -Name $SelectCollectionName -ErrorAction SilentlyContinue
            ...
            foreach ($ExcludeCollectionName in $ExcludeCollectionNames) {
                try {
                    $ExcludeCollection = Get-CMDeviceCollection -Name $ExcludeCollectionName -ErrorAction SilentlyContinue
                    if ($ExcludeCollection) {
                        Write-PCXLog "Adding exclusion collection '$ExcludeCollectionName' to '$SelectCollectionName'"
                        $null = Add-CMDeviceCollectionExcludeMembershipRule -CollectionId $TargetCollection.CollectionId -ExcludeCollectionId $ExcludeCollection.CollectionId
                    ...
                    Write-PCXLog "Failed to add exclusion collection '$ExcludeCollectionName' to '$SelectCollectionName'. $_"
                }
            }
        }
        catch {
            Write-PCXLog "Failed to process exclusion collections for '$SelectCollectionName'. $_"
            throw
        }
        finally {
            Write-PCXLog "Collection exclusion operation completed: $SelectCollectionName"
        }
    }
    end {
