function New-PCXCMApplicationDeployment {
    ...
    process {
        try {
            Write-PCXLog "Creating deployment for: $Name"
            Write-PCXLog "Collection: $CollectionName ($Purpose/$Action)"
            ...
            if ($Purpose -eq "Required") {
                $DeploymentParams["DeadlineDateTime"] = $DeadlineDateTime
            }

            $null = New-CMApplicationDeployment @DeploymentParams

            Write-PCXLog "Application deployment created successfully"
        }
        catch {
