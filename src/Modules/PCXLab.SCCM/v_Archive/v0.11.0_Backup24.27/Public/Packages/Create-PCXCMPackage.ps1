function Create-PCXCMPackage {
    ...
    process {
        ...
        try {
            ...
            <#
            # Set meetered network for install
            Set-PCXCMPackageDeployment `
                -PackageName $PackageName `
                -ProgramName $Programs.Install `
                -CollectionName $Collections.Install `
                -UseMeteredNetwork $true

            # Set meetered network for Uninstall
            Set-PCXCMPackageDeployment `
                -PackageName $PackageName `
            ...
            Set-PCXCMPackageDeployment `
                ...
                -ProgramName "PKG Igor Pavlov 7zip 26.0.0 [Uninstall]" `
                -CollectionName "PKG Igor Pavlov 7zip 26.0.0 [Uninstall]" `
                -UseMeteredNetwork $true
                #>
