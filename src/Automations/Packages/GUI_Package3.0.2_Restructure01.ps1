...
function Create-Package { 
    ...
    # ---------------------------------------
    # $Path = "\\192.168.25.214\Package_Source\Applications\Google\Chrome\Chrome_144.0.7547.0"
    $PackageMetadata = Get-PCXPackageInfoFromPath -Path $Path
    ...
    Write-Host "Package name: $($PackageMetadata.Packagename)" -ForegroundColor Yellow
    Write-Host "Company/Manufacturer:  $($PackageMetadata.Company)" -ForegroundColor Yellow
    Write-Host "Product/Application:  $($PackageMetadata.Product)" -ForegroundColor Yellow

    $PackageTest = Get-CMPackage -Name $($PackageMetadata.Packagename)
