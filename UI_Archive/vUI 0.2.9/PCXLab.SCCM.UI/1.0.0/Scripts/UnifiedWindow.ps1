...
function Show-PCXCMConfirmDialog {
    ...
    $cBorderHeader = $cWindow.FindName("borderConfHeader")
    ...
    $cBorderHeader.Add_MouseLeftButtonDown({ $cWindow.DragMove() })
...
# Unified Creation Action (Non-Blocking Background Runspace)
$btnCreate.Add_Click({
    ...
    $ObjectName = if ($IsApp) { $txtAppName.Text } else { $txtPkgName.Text }
    ...
    # ------------------------------------------------------------------
    # Capture all values needed by the background thread BEFORE launch
    # ------------------------------------------------------------------
    $script:AsyncType            = if ($IsApp) { "APPLICATION" } else { "PACKAGE" }
    $script:AsyncObjectName      = $ObjectName
    $script:AsyncPath            = $Path
    $script:AsyncRefNumber       = $txtRefNumber.Text
    $script:AsyncReviewerName    = $txtReviewer.Text
    $script:AsyncComments        = $txtComments.Text
    $script:AsyncSelectedGroups  = $SelectedGroups
    $script:AsyncDistPoints      = $DistributionPoints
    $script:AsyncIsApp           = $IsApp

    # Retrieve SCCM PSDrive details in main thread (required before runspace)
    $CMSiteDrive = Get-PSDrive -PSProvider CMSite -ErrorAction SilentlyContinue | Select-Object -First 1
    $script:AsyncSiteCode        = if ($CMSiteDrive) { $CMSiteDrive.Name } else { $null }
    $script:AsyncProviderMachine = if ($CMSiteDrive) { $CMSiteDrive.Root } else { $null }

    # Capture the module root for import inside the runspace
    $script:AsyncModulePath      = $UIFunctionsPath

    # ------------------------------------------------------------------
    # Disable button, set wait cursor
    # ------------------------------------------------------------------
    $btnCreate.IsEnabled = $false
    $Window.Cursor = [System.Windows.Input.Cursors]::Wait
    Write-GUIStatus ">>> STARTING $($script:AsyncType) CREATION: $($script:AsyncObjectName)" "ACTION"

    # ------------------------------------------------------------------
    # Build and launch the background runspace
    # ------------------------------------------------------------------
    $script:AsyncRunspace = [System.Management.Automation.Runspaces.RunspaceFactory]::CreateRunspace()
    $script:AsyncRunspace.ApartmentState = [System.Threading.ApartmentState]::STA
    $script:AsyncRunspace.ThreadOptions  = [System.Management.Automation.Runspaces.PSThreadOptions]::ReuseThread
    $script:AsyncRunspace.Open()

    # Pass captured values into the runspace via session state variables
    $script:AsyncRunspace.SessionStateProxy.SetVariable("AsyncType",            $script:AsyncType)
    $script:AsyncRunspace.SessionStateProxy.SetVariable("AsyncObjectName",      $script:AsyncObjectName)
    $script:AsyncRunspace.SessionStateProxy.SetVariable("AsyncPath",            $script:AsyncPath)
    $script:AsyncRunspace.SessionStateProxy.SetVariable("AsyncRefNumber",       $script:AsyncRefNumber)
    $script:AsyncRunspace.SessionStateProxy.SetVariable("AsyncReviewerName",    $script:AsyncReviewerName)
    $script:AsyncRunspace.SessionStateProxy.SetVariable("AsyncComments",        $script:AsyncComments)
    $script:AsyncRunspace.SessionStateProxy.SetVariable("AsyncSelectedGroups",  $script:AsyncSelectedGroups)
    $script:AsyncRunspace.SessionStateProxy.SetVariable("AsyncDistPoints",      $script:AsyncDistPoints)
    $script:AsyncRunspace.SessionStateProxy.SetVariable("AsyncIsApp",           $script:AsyncIsApp)
    $script:AsyncRunspace.SessionStateProxy.SetVariable("AsyncSiteCode",        $script:AsyncSiteCode)
    $script:AsyncRunspace.SessionStateProxy.SetVariable("AsyncProviderMachine", $script:AsyncProviderMachine)
    $script:AsyncRunspace.SessionStateProxy.SetVariable("AsyncModulePath",      $script:AsyncModulePath)

    $BackgroundScript = {
        # Load helpers from the UI Functions folder
        Get-ChildItem -Path "$AsyncModulePath\*.ps1" -ErrorAction SilentlyContinue | ForEach-Object { . $_.FullName }

        # Re-connect to SCCM if site info was captured
        if ($AsyncSiteCode -and $AsyncProviderMachine) {
            try { Connect-PCXCMSite -SiteCode $AsyncSiteCode -ProviderMachineName $AsyncProviderMachine -ErrorAction SilentlyContinue } catch {}
        }

        $Params = @{
            Path            = $AsyncPath
            ReferenceNumber = $AsyncRefNumber
            ReviewerName    = $AsyncReviewerName
            Comments        = $AsyncComments
        }

        if ($AsyncSelectedGroups -and $AsyncSelectedGroups.Count -gt 0) {
            $Params["DistributionPointGroups"] = $AsyncSelectedGroups
        }
        if ($AsyncDistPoints -and $AsyncDistPoints.Count -gt 0) {
            $Params["DistributionPoints"] = $AsyncDistPoints
        }
    ...
    $script:AsyncPS = [System.Management.Automation.PowerShell]::Create()
    $script:AsyncPS.Runspace = $script:AsyncRunspace
    [void]$script:AsyncPS.AddScript($BackgroundScript)
    $script:AsyncResult = $script:AsyncPS.BeginInvoke()

    # ------------------------------------------------------------------
    # DispatcherTimer — polls every 200ms, streams output, detects done
    # ------------------------------------------------------------------
    $script:AsyncTimer = New-Object System.Windows.Threading.DispatcherTimer
    $script:AsyncTimer.Interval = [TimeSpan]::FromMilliseconds(200)

    $script:AsyncTimer.Add_Tick({
        # Drain Verbose stream
        while ($script:AsyncPS.Streams.Verbose.Count -gt 0) {
            $msg = $script:AsyncPS.Streams.Verbose[0].Message
            $script:AsyncPS.Streams.Verbose.RemoveAt(0)
            Write-GUIStatus $msg "VERBOSE"
        }
        # Drain Information stream
        while ($script:AsyncPS.Streams.Information.Count -gt 0) {
            $msg = $script:AsyncPS.Streams.Information[0].MessageData
            $script:AsyncPS.Streams.Information.RemoveAt(0)
            Write-GUIStatus $msg "INFO"
        }
        # Drain Warning stream
        while ($script:AsyncPS.Streams.Warning.Count -gt 0) {
            $msg = $script:AsyncPS.Streams.Warning[0].Message
            $script:AsyncPS.Streams.Warning.RemoveAt(0)
            Write-GUIStatus $msg "WARNING"
        }
        # Drain Error stream
        while ($script:AsyncPS.Streams.Error.Count -gt 0) {
            $msg = $script:AsyncPS.Streams.Error[0].ToString()
            $script:AsyncPS.Streams.Error.RemoveAt(0)
            Write-GUIStatus $msg "ERROR"
        }
        ...
        if ($script:AsyncResult.IsCompleted) {
            ...
            # Collect final errors from EndInvoke
            $FinalErrors = $null
            try {
                [void]$script:AsyncPS.EndInvoke($script:AsyncResult)
            } catch {
                $FinalErrors = $_.Exception.Message
            }
            ...
            if ($FinalErrors) {
                Write-GUIStatus "FAILED: $FinalErrors" "ERROR"
                [System.Windows.MessageBox]::Show("Creation failed:`r`n$FinalErrors", "Error")
            } else {
                Write-GUIStatus "SUCCESS: $($script:AsyncType) created and distributed." "SUCCESS"
                [System.Windows.MessageBox]::Show("$($script:AsyncType) created successfully.", "Success")
            }
        }
    })
