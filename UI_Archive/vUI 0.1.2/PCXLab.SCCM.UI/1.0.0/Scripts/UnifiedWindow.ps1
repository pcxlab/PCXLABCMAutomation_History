...
function Refresh-DistributionLists {
    ...
    $Window.Dispatcher.Invoke({
        ...
        try {
            ...
            Write-GUIStatus "Successfully loaded $($Groups.Count) Groups and $($DPs.Count) DPs."
        }
        catch {
...
$btnCreate.Add_Click({
    ...
    if (-not $Path) { [System.Windows.MessageBox]::Show("Please select a source path.", "Error"); return }

    $SelectedGroups = Get-SelectedItemsFromPanel -Panel $pnlDPGroups
    $SelectedDPs = Get-SelectedItemsFromPanel -Panel $pnlDPs
    
    if ($SelectedGroups.Count -eq 0 -and $SelectedDPs.Count -eq 0) {
        [System.Windows.MessageBox]::Show("Please select at least one Target (DP Group or DP).", "Error"); return
    }
    ...
    try {
        ...
        if ($SelectedGroups.Count -gt 0) { $Params["DistributionPointGroups"] = $SelectedGroups }
        if ($SelectedDPs.Count -gt 0) { $Params["DistributionPoints"] = $SelectedDPs }
