function Get-PCXCMCMGs {
    ...
    try {
        ...
        if ($null -eq $CMGData -and (Get-Command Get-CMCloudManagementGateway -ErrorAction SilentlyContinue)) {
            $CMGData = Get-CMCloudManagementGateway -ErrorAction SilentlyContinue
        }
