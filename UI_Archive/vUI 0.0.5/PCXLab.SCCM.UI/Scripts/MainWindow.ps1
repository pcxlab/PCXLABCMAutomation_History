...
$lstDPGroups = $Window.FindName("lstDPGroups")
...
foreach ($DPGroup in $DPGroups) {
    [void]$lstDPGroups.Items.Add($DPGroup)
}

if ($lstDPGroups.Items.Count -gt 0) {
    $lstDPGroups.SelectedIndex = 0
}
...
$btnCreatePackage.Add_Click({
        ...
        try {
          ...
          Write-GUIStatus "Testing selected DP Groups..."

$SelectedDPGroups = @($lstDPGroups.SelectedItems)

if ($SelectedDPGroups.Count -eq 0) {

    [System.Windows.MessageBox]::Show(
        "Please select at least one DP Group.",
        "Validation Error"
    )
...
Write-GUIStatus "Selected DP Groups:"

$SelectedDPGroups | ForEach-Object {
    Write-GUIStatus " - $_"
}

[System.Windows.MessageBox]::Show(
    ($SelectedDPGroups -join "`r`n"),
    "Selected DP Groups"
)
