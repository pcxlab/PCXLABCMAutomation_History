...
# V4.4 Final Stable Patched Edition
#
# Goal:
# - Keep V4.4 structure
# - Restore missed Core module seeded files
# - Fill version folders consistently
# - Keep GitHub + tools + docs + guidance
# - Ready for long-term real development
# =====================================================================
...
0.0.0 = prototype baseline
0.1.0 = stable release
"@

        # -------------------------------------------------------------
        # CORE MODULE CONTENT (ALL versions)
        # -------------------------------------------------------------
        if ($Module -eq "PCXLab.Core") {
...
'@
        ...
        # -------------------------------------------------------------
        # SCCM MODULE CONTENT (ALL versions)
        # -------------------------------------------------------------
        if ($Module -eq "PCXLab.SCCM") {
...
# Root Docs / Notes
# ---------------------------------------------------------------------
Set-PCXFile "$ProjectRoot\README.md" "# PCXCMAutomation"
Set-PCXFile "$ProjectRoot\CHANGELOG.md" "# Changelog`n`n## [4.4 Final Stable]"
...
Set-PCXFile "$ProjectRoot\docs\templates\AutomationTemplate.ps1" 'Write-Host "Automation Template"'
Set-PCXFile "$ProjectRoot\docs\templates\RunbookTemplate.ps1" 'Write-Host "Runbook Template"'
...
Set-PCXFile "$ProjectRoot\ops\logs\README.md" "Logs here"
Set-PCXFile "$ProjectRoot\ops\reports\README.md" "Reports here"
Set-PCXFile "$ProjectRoot\ops\artifacts\README.md" "Artifacts here"

# ---------------------------------------------------------------------
# Tools
# ---------------------------------------------------------------------
Set-PCXFile "$ProjectRoot\tools\README.md" "Helper utilities"
Set-PCXFile "$ProjectRoot\tools\bootstrap\README.md" "Bootstrap tools"
Set-PCXFile "$ProjectRoot\tools\signing\README.md" "Signing tools"
Set-PCXFile "$ProjectRoot\tools\packaging\README.md" "Packaging tools"
Set-PCXFile "$ProjectRoot\tools\helpers\README.md" "Helper tools"

# ---------------------------------------------------------------------
# GitHub
# ---------------------------------------------------------------------
Set-PCXFile "$ProjectRoot\.github\workflows\README.md" "CI/CD workflows"
Set-PCXFile "$ProjectRoot\.github\PULL_REQUEST_TEMPLATE.md" "Describe changes."
Set-PCXFile "$ProjectRoot\.github\ISSUE_TEMPLATE\bug_report.md" "Bug report"
Set-PCXFile "$ProjectRoot\.github\ISSUE_TEMPLATE\feature_request.md" "Feature request"
...
Write-Host "PCXCMAutomation V4.4 Final Stable Ready" -ForegroundColor Green
